...
Complete traceability matrix mapping every EIP to its specification, implementation, tests, and roadmap position. eth2028 implements **94+ EIPs** across all upgrade phases.
...
The `pkg/geth/` adapter package bridges eth2028 to go-ethereum v1.17.0:
...
| Pre-state | `geth/prestate.go` | Create go-ethereum StateDB from eth2028 accounts |
...
| Custom precompiles | `geth/extensions.go` | 13 eth2028 precompiles injected via SetPrecompiles() |
...
**Note**: go-ethereum's `operation` struct is unexported, so 26 custom opcodes (CLZ, DUPN, APPROVE, TXPARAM*, EOF, AA) remain eth2028-native-EVM-only.
