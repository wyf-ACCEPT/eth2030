...
func (s *MLDSASigner) GenerateKey() (*MLDSAKeyPair, error) {
	...
	for j := 0; j < mldsaL; j++ {
		s1[j] = mldsaSampleCBD(rhoPrime, uint16(j))
	}
	s2 := make([]mldsaPoly, mldsaK)
	for i := 0; i < mldsaK; i++ {
		s2[i] = mldsaSampleCBD(rhoPrime, uint16(mldsaL+i))
	}
	...
	for i := 0; i < mldsaK; i++ {
		t1[i], t0[i] = mldsaPower2Round(t[i])
	}
...
func (s *MLDSASigner) Sign(key *MLDSAKeyPair, msg []byte) ([]byte, error) {
	...
	for kappa := 0; kappa < 2048; kappa++ {
		...
		for j := 0; j < mldsaL; j++ {
			y[j] = mldsaSampleGamma1(rhoPrime, uint16(kappa*mldsaL+j))
		}
		w := mldsaMatVecMul(key.aMatrix, y)
		w1 := make([]mldsaPoly, mldsaK)
		for i := 0; i < mldsaK; i++ {
			w1[i] = mldsaHighBits(w[i])
		}
		...
		for j := 0; j < mldsaL; j++ {
			...
			if !mldsaCheckNorm(z[j], mldsaGamma1-mldsaBeta) {
				reject = true
				break
			}
		}
		...
		for i := 0; i < mldsaK; i++ {
			...
			if !mldsaCheckNorm(r0, mldsaGamma2-mldsaBeta) {
				lowReject = true
				break
			}
		}
		...
		for len(sig) < MLDSASignatureSize {
			sig = append(sig, 0)
		}
...
func (s *MLDSASigner) Verify(pk, msg, sig []byte) bool {
	if len(pk) < MLDSAPublicKeySize || len(sig) < MLDSASignatureSize || len(msg) == 0 {
		return false
	}
...
func mldsaSerializeSK(rho, kSeed, tr []byte, s1, s2, t0 []mldsaPoly) []byte {
	...
	for len(sk) < MLDSAPrivateKeySize {
		sk = append(sk, 0)
	}
...
func mldsaUnpackPoly(data []byte) mldsaPoly {
	var p mldsaPoly
	for i := 0; i < mldsaN && 4*(i+1) <= len(data); i++ {
...
func mldsaMatVecMul(a [][]mldsaPoly, s []mldsaPoly) []mldsaPoly {
	...
	for i := 0; i < mldsaK; i++ {
		for j := 0; j < mldsaL; j++ {
			t[i] = mldsaPolyAdd(t[i], mldsaPolyMul(a[i][j], s[j]))
		}
	}
	return t
}
...
func mldsaSampleCBD(seed []byte, nonce uint16) mldsaPoly {
	...
	for i := 0; i < mldsaN; i++ {
		p[i] = int64(stream[2*i])%int64(2*mldsaEta+1) - mldsaEta
	}
	return p
}
